...
def compute_subswath_xspectra(dt):
    ...
    dt = datatree.DataTree.from_dict({'interburst_xspectra':netcdf_compliant(inter_xs), 'intraburst_xspectra':netcdf_compliant(intra_xs)})
    return dt


def compute_subswath_intraburst_xspectra(dt, tile_width={'sample':20.e3, 'line':20.e3}, tile_overlap={'sample':10.e3, 'line':10.e3}, **kwargs):
    """
...
def compute_subswath_interburst_xspectra(dt, tile_width={'sample':20.e3, 'line':20.e3}, tile_overlap={'sample':10.e3, 'line':10.e3}, **kwargs):
    ...
    for b in range(dt['bursts'].sizes['burst']-1):
        ...
        interburst_xspectra = tile_bursts_overlap_to_xspectra(burst0, burst1, tile_width, tile_overlap, azimuth_steering_rate, azimuth_time_interval)
...
def tile_burst_to_xspectra(burst, tile_width, tile_overlap, radar_frequency, lowpass_width = {'sample':1000., 'line':1000.}, **kwargs):
    """
    ...
    for i in xndindex(tiles_sizes):
        ...
        mod = compute_modulation(sub['deramped_digital_number'], lowpass_width=lowpass_width, spacing={'sample':ground_spacing, 'line':azimuth_spacing})
        xspecs = compute_intraburst_xspectrum(mod, mean_incidence, slant_spacing, azimuth_spacing, synthetic_duration, **kwargs)
        xspecs_m = xspecs.mean(dim=['periodo_line', 'periodo_sample'], keep_attrs=True) # averaging all the periodograms in each tile
        xs[tuple(i.values())] = xspecs_m
        # ------------- tau ----------------
        taus[i] = float(xspecs.attrs['tau'])
        # ------------- cut-off ------------
        cutoff_tau = [str(i)+'tau' for i in [1,2,3,0] if str(i)+'tau' in xspecs_m.dims][0] # which tau is used to compute azimuthal cutoff
        k_rg = xspecs_m.k_rg
        k_az = xspecs_m.k_az
        xspecs_m = xspecs_m['xspectra_'+cutoff_tau].mean(dim=cutoff_tau)
        xspecs_m = xspecs_m.assign_coords({'k_rg':k_rg, 'k_az':k_az}).swap_dims({'freq_sample':'k_rg', 'freq_line':'k_az'})
    ...
    xs = xr.merge([xs, taus.to_dataset(), cutoff.to_dataset(), corner_lon.to_dataset(), corner_lat.to_dataset()], combine_attrs='drop_conflicts')
    xs = xs.assign_coords({'longitude':middle_lon, 'latitude':middle_lat}) # This line also ensures adding line/sample coordinates too !! DO NOT REMOVE
...
def crop_burst(ds, burst_annotation, burst_number, valid=True, merge_burst_annotation = True):
    ...
    if valid:
        fs, ls, fl, ll = burst_valid_indexes(burst_annotation.sel(burst=burst_number)) # first and last line are relative to burst
    else:    
        fs, ls = None,None
...
def deramp_burst(burst, dt):
    ...
    """
    from sar_slc.deramping import compute_midburst_azimuthtime, compute_slant_range_time, compute_Doppler_centroid_rate, compute_reference_time, compute_deramping_phase, compute_DopplerCentroid_frequency
    ...
    slant_range_time = compute_slant_range_time(burst, dt['image'].ds['slantRangeTime'], dt['image'].ds['rangeSamplingRate'])

    kt = compute_Doppler_centroid_rate(orbit, azimuth_steering_rate, radar_frequency, FMrate, midburst_azimuth_time, slant_range_time)
    fnc = compute_DopplerCentroid_frequency(dcEstimates, midburst_azimuth_time, slant_range_time)
    eta_ref = compute_reference_time(FMrate, dcEstimates, midburst_azimuth_time, slant_range_time, int(burst['samplesPerBurst']))
...
def compute_modulation(ds, lowpass_width, spacing):
    ...
    low_pass_intensity = xr.apply_ufunc(fftconvolve, np.abs(ds.where(mask, 0.))**2, gk, input_core_dims=[lowpass_width.keys(), swap_dims.values()], vectorize=True, output_core_dims=[lowpass_width.keys()], kwargs={'mode':'same'})

    normal = xr.apply_ufunc(fftconvolve, mask, gk, input_core_dims=[lowpass_width.keys(), swap_dims.values()], vectorize=True, output_core_dims=[lowpass_width.keys()], kwargs={'mode':'same'})
...
def compute_intraburst_xspectrum(slc, mean_incidence, slant_spacing, azimuth_spacing, synthetic_duration, azimuth_dim='line', nperseg={'sample':512, 'line':512}, noverlap={'sample':256, 'line':256}, **kwargs):
    """
    ...
    for i in xndindex(periodo_sizes):
        image = periodo[i]
        xspecs = compute_looks(image, azimuth_dim=azimuth_dim, synthetic_duration=synthetic_duration, **kwargs)#.assign_coords(i)
    ...
    k_az = xr.DataArray(np.fft.fftshift(np.fft.fftfreq(out.sizes['freq_'+azimuth_dim], azimuth_spacing/(out.attrs.pop('look_width')*2*np.pi))), dims='freq_'+azimuth_dim, name = 'k_az', attrs={'long_name':'wavenumber in azimuth direction', 'units':'rad/m'})
    # out = out.assign_coords({'k_rg':k_rg, 'k_az':k_az}).swap_dims({'freq_'+range_dim:'k_rg', 'freq_'+azimuth_dim:'k_az'})
    out = xr.merge([out, k_rg.to_dataset(), k_az.to_dataset()], combine_attrs='drop_conflicts') # Adding .to_dataset() ensures promote_attrs=False
    out.attrs.update({'periodogram_nperseg_'+range_dim:nperseg[range_dim], 'periodogram_nperseg_'+azimuth_dim:nperseg[azimuth_dim], 'periodogram_noverlap_'+range_dim:noverlap[range_dim], 'periodogram_noverlap_'+azimuth_dim:noverlap[azimuth_dim]})

    return out.drop(['freq_'+range_dim, 'freq_'+azimuth_dim])


def compute_looks(slc, azimuth_dim, synthetic_duration, nlooks=3, look_width=0.2, look_overlap=0., look_window=None, plot=False):
    """
    ...
    mydop = xrft.fft(slc, dim=[azimuth_dim], detrend=None, window=None, shift=True, true_phase=True, true_amplitude=True)
    
    # Finding an removing Doppler centroid
    weight = xr.DataArray(np.hanning(100), dims=['window']) # window for smoothing
    weight/=weight.sum()
    smooth_dop = np.abs(mydop).mean(dim=range_dim).rolling(**{freq_azi_dim:len(weight), 'center':True}).construct('window').dot(weight)
    ...
    for l in xndindex(looks_sizes):
        look = looks[l]
        look = xrft.ifft(look.assign_coords({freq_azi_dim:np.arange(-(nperlook//2),-(nperlook//2)+nperlook)*float(mydop[freq_azi_dim].spacing)}), dim=freq_azi_dim, detrend=None, window=False, shift=True, true_phase=False, true_amplitude=True)
        look = np.abs(look)**2
    ...
    for i in range(nlooks):
        ...
        concat_spec.attrs.update({'nlooks':nlooks, 'look_width':look_width, 'look_overlap':look_overlap, 'look_window':str(look_window), 'tau':tau})
...
def tile_bursts_overlap_to_xspectra(burst0, burst1, tile_width, tile_overlap, azimuth_steering_rate, azimuth_time_interval, lowpass_width = {'sample':1000., 'line':1000.}, **kwargs):
    """
    ...
    for i in xndindex(tiles_sizes):
        ...
        mod0 = compute_modulation(np.abs(sub0['digital_number']), lowpass_width=lowpass_width, spacing={'sample':ground_spacing, 'line':azimuth_spacing})
        mod1 = compute_modulation(np.abs(sub1['digital_number']), lowpass_width=lowpass_width, spacing={'sample':ground_spacing, 'line':azimuth_spacing})
        
        xspecs = compute_interburst_xspectrum(mod0**2, mod1**2, mean_incidence, slant_spacing, azimuth_spacing, **kwargs)
        xspecs_m = xspecs.mean(dim=['periodo_line', 'periodo_sample'], keep_attrs=True) # averaging all the periodograms in each tile
        xs[tuple(i.values())] = xspecs_m
        # ------------- tau ------------------
        ...
        dist0 = (sub0[{'line':sub0.sizes['line']//2}]['line']-sub0['linesPerBurst']*sub0['burst'])*azimuth_spacing # distance from begining of the burst
        dist1 = (sub1[{'line':sub1.sizes['line']//2}]['line']-sub1['linesPerBurst']*sub1['burst'])*azimuth_spacing # distance from begining of the burst
        tau = (sub1['sensingTime']-sub0['sensingTime'])/np.timedelta64(1,'s')+(dist1-dist0)/scan_velocity # The division by timedelta64(1,s) is to convert in seconds
        taus[i] = tau.item()
        # ------------- cut-off --------------
        k_rg = xspecs_m.k_rg
        k_az = xspecs_m.k_az
...
def compute_interburst_xspectrum(mod0, mod1, mean_incidence, slant_spacing, azimuth_spacing, azimuth_dim='line', nperseg={'sample':512, 'line':None}, noverlap={'sample':256,'line':0}):
    """
    ...
    out = [list(a) for a in list(out)] # must be generalized for larger number of dimensions
    out = xr.combine_nested(out, concat_dim=periodo_sizes.keys(), combine_attrs='drop_conflicts').rename('interburst_xspectra')
    ...
    ground_range_spacing = slant_spacing/np.sin(np.radians(out.mean_incidence))
    k_rg = xr.DataArray(np.fft.fftshift(np.fft.fftfreq(out.sizes['freq_'+range_dim], ground_range_spacing/(2*np.pi))), dims='freq_'+range_dim, name = 'k_rg', attrs={'long_name':'wavenumber in range direction', 'units':'rad/m'})
    k_az = xr.DataArray(np.fft.fftshift(np.fft.fftfreq(out.sizes['freq_'+azimuth_dim], azimuth_spacing/(2*np.pi))), dims='freq_'+azimuth_dim, name = 'k_az', attrs={'long_name':'wavenumber in azimuth direction', 'units':'rad/m'})
    # out = out.assign_coords({'k_rg':k_rg, 'k_az':k_az}).swap_dims({'freq_'+range_dim:'k_rg', 'freq_'+azimuth_dim:'k_az'})
    out = xr.merge([out, k_rg.to_dataset(), k_az.to_dataset()], combine_attrs='drop_conflicts') # Adding .to_dataset() ensures promote_attrs=False
    out.attrs.update({'periodogram_nperseg_'+range_dim:nperseg[range_dim], 'periodogram_nperseg_'+azimuth_dim:nperseg[azimuth_dim], 'periodogram_noverlap_'+range_dim:noverlap[range_dim], 'periodogram_noverlap_'+azimuth_dim:noverlap[azimuth_dim]})
    return out
...
# def compute_SAR_cross_spectrum(slc, mean_incidence, slant_spacing, azimuth_spacing, synthetic_duration, azimuth_dim='line', nperseg={'sample':512, 'line':256}, noverlap={'sample':256, 'line':128}, **kwargs):
#     """
#     Compute SAR cross spectrum using a 2D Welch method. Looks are centered on the mean Doppler frequency
#     If ds contains only one cycle, spectrum wavenumbers are added as coordinates in returned DataSet, othrewise, they are passed as variables (k_range, k_azimuth).
...
#     periodo_slices = xtiling(slc, nperseg = nperseg, noverlap=noverlap)

#     out = np.empty(periodo_slices.shape, dtype=object)
#     # out = xr.ones_like(periodo_slices, dtype=object)
#     # out=list()

#     # for i in np.ndindex(periodo_slices.shape):
#     #     image = slc[periodo_slices[{d:j for d,j in zip(periodo_slices.dims,i)}].item()]
#     for i in xndindex(periodo_slices.sizes):
#         image = slc[periodo_slices[i].item()]
#         # xspecs = compute_looks(image, azimuth_dim, synthetic_duration, **kwargs).assign_coords({d:j for d,j in zip(periodo_slices.dims,i)})
#         xspecs = compute_looks(image, azimuth_dim, synthetic_duration, **kwargs).assign_coords(i)
#         # out.append(xspecs.assign_coords(i))
#         # print(i,xspecs)
#         ii,jj = list(i.values()) # shitty trick
#         out[ii,jj] = xspecs
#     # print(out)
#     # out = np.array(out).reshape(periodo_slices.shape)
#     out = [list(a) for a in list(out)] # must be generalized for larger number of dimensions
#     out = xr.combine_nested(out, concat_dim=periodo_slices.dims, combine_attrs='drop_conflicts')
#     out = out.assign_coords(periodo_slices.coords)
...
#     ground_range_spacing = slant_spacing/np.sin(np.radians(out.mean_incidence))
#     k_rg = xr.DataArray(np.fft.fftshift(np.fft.fftfreq(out.sizes['freq_'+range_dim], ground_range_spacing/(2*np.pi))), dims='freq_'+range_dim, name = 'k_rg', attrs={'long_name':'wavenumber in range direction', 'units':'rad/m'})
#     k_az = xr.DataArray(np.fft.fftshift(np.fft.fftfreq(out.sizes['freq_'+azimuth_dim], azimuth_spacing/(out.look_width*2*np.pi))), dims='freq_'+azimuth_dim, name = 'k_az', attrs={'long_name':'wavenumber in azimuth direction', 'units':'rad/m'})
#     # out = out.assign_coords({'k_rg':k_rg, 'k_az':k_az}).swap_dims({'freq_'+range_dim:'k_rg', 'freq_'+azimuth_dim:'k_az'})
#     out = xr.merge([out, k_rg, k_az], combine_attrs='drop_conflicts')
#     out.attrs.update({'periodogram_nperseg_'+range_dim:nperseg[range_dim], 'periodogram_nperseg_'+azimuth_dim:nperseg[azimuth_dim], 'periodogram_noverlap_'+range_dim:noverlap[range_dim], 'periodogram_noverlap_'+azimuth_dim:noverlap[azimuth_dim]})
...
# def xtiling(ds, nperseg, noverlap = 0, centering=False, prefix='tile_', side='left'):
#     """
#     Define tiles indexes of an xarray of abritrary shape. Note that coordinates of returned array depends on type of ds.
...
#     Name of returned coordinate are prefix+nperseg keys()
...
#     Return:
#         (xarray.DataArray): array of slices whose dimensions are nperseg keys (or ds dimensions if nperseg is an integer).
#     """
...
#     mat = np.empty([len(ind) for d,ind in indices.items()],dtype=dict)
    
#     for ind in np.ndindex(mat.shape):
#         mat[ind] = {d:slice(indices[d][ind[n]], indices[d][ind[n]]+nperseg[d]) for n,d in enumerate(dims)}
    
    
#     if isinstance(ds, xr.DataArray) or isinstance(ds, xr.Dataset):
#         coords = {d:ds[d][indices[d]]+nperseg[d]//2 for d in dims}
#     else:
#         coords = {d:indices[d]+nperseg[d]//2 for d in dims}
                  
#     res = xr.DataArray(mat, dims=dims, coords=coords)
#     for c in res.coords:
#         res.coords[c].attrs.update({'long_name': 'index of tile middle point'})
#     for d in res.dims:
#         res[d].attrs.update({'nperseg':nperseg[d], 'noverlap':noverlap[d]})

#     return res.rename('tiles')



# def xndindex(ds, dims=None):
#     """
#     Iterator over xarray.
    
#     Args:
#         ds (xarray.DataArray or dataset)
#         dims (list of str, optional): list of dimensions to iterate on. Default is all dimension of ds
#     Return:
