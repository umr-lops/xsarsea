...
def invert_from_model(
    ...
    **kwargs,
    ...
    if sigma0_dual is None:
        ...
        if pol is None:
            warnings.warn(f"Unable to check sigma0 pol. Assuming  {model_pol}")
        else:
            if pol not in model_pol:
                raise ValueError(
                    f"sigma0 pol is {pol}, and model {models[0].name} can only handle {model_pol}"
                )
    ...
    def _invert_from_model_numpy(
        np_inc, np_sigma0_co_db, np_sigma0_cr_db, np_dsig_cr, np_ancillary_wind
    ):
        ...
        try:
            ...
            np_sigma0_co_lut_db = np.ascontiguousarray(
                np.asarray(sigma0_co_lut_db.transpose("wspd", "phi", "incidence"))
            )
        ...
        def __invert_from_model_1d(
            inc_1d, sigma0_co_db_1d, sigma0_cr_db_1d, dsig_cr_1d, ancillary_wind_1d, out_co, out_cr
        ):
            ...
            for i in range(len(inc_1d)):
                ...
                if not np.isnan(one_sigma0_co_db):
                    ...
                    Jwind_co = ((np_wspd_lut_co_antenna - m_antenna) / d_antenna) ** 2 + (
                        (np_wspd_lut_co_azi - m_azi) / d_azi
                    ) ** 2  # shape (phi, wspd)
                    # shape (wspd, phi)
                    Jsig_co = ((np_sigma0_co_lut_db_inc - one_sigma0_co_db) / dsig_co) ** 2
                    ...
                    if phi_180:
                        ...
                        sol_2 = wspd_co * np.exp(1j * (np.deg2rad(-wphi_co)))

                        diff_angle = np.angle(one_ancillary_wind / sol)
                        diff_angle_2 = np.angle(one_ancillary_wind / sol_2)

                        wind_co = sol if np.abs(diff_angle) <= np.abs(diff_angle_2) else sol_2
                ...
                if not np.isnan(one_sigma0_cr_db) and not np.isnan(one_dsig_cr):
                    ...
                    Jwind_cr = ((np_wspd_lut_cr - np.abs(wind_co)) / dwspd_fg) ** 2.0
                    Jsig_cr = ((np_sigma0_cr_lut_db_inc - one_sigma0_cr_db) / one_dsig_cr) ** 2.0
        ...
        with warnings.catch_warnings():
            ...
            return __invert_from_model_vect(
                np_inc, np_sigma0_co_db, np_sigma0_cr_db, np_dsig_cr, np_ancillary_wind
            )
    ...
    if sigma0_dual is None:
        ...
        # dualpol inversion
        wspd_dual = xr.where(
            (np.abs(ws_co) < 5) | (np.abs(ws_cr_or_dual) < 5), ws_co, ws_cr_or_dual
        )
        # wspd_dual = ws_cr_or_dual
        try:
            wspd_dual.attrs["comment"] = (
                f"wind speed and direction inverted from model {models[0].name} ({models[0].pol}) and {models[1].name} ({models[1].pol})"
            )
            wspd_dual.attrs["model"] = f"{models[0].name} {models[1].name}"
